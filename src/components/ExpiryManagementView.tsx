    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm flex flex-col lg:flex-row lg:items-center gap-6">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 transition-all group">
             ...
             <span className="text-[11px] font-black text-slate-500 tracking-tight">{t('expiry.validity_filter')}</span>
          ...
          <div className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 transition-all">
             <span className="text-[11px] font-black text-slate-500 tracking-tight">{t('expiry.status_filter')}</span>
             <ChevronDown className="w-3.5 h-3.5 text-slate-400" />
          </div>
        </div>
      ...
      <div className="flex items-center gap-8 px-4 py-2">
        <label className="flex items-center gap-3 cursor-pointer group">
          ...
          <span className="text-[11px] font-black text-slate-400 tracking-widest group-hover:text-slate-600 transition-colors">{t('expiry.show_in_stock')}</span>
        </label>
        <label className="flex items-center gap-3 cursor-pointer group">
          <div className="w-5 h-5 border-2 border-slate-200 rounded-lg group-hover:border-primary transition-all flex items-center justify-center bg-white shadow-sm">
             <div className="w-2.5 h-2.5 rounded-[2px] bg-primary scale-0 group-hover:scale-100 transition-transform"></div>
          </div>
          <span className="text-[11px] font-black text-slate-400 tracking-widest group-hover:text-slate-600 transition-colors">{t('expiry.show_processed_expired')}</span>
        </label>
      </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 tracking-widest text-right">{t('common.actions')}</th>
              </tr>
            </thead>
              ...
              {inventory.map((row, idx) => (
                <tr key={idx} className="hover:bg-slate-50/80 transition-all duration-200 group">
                  ...
                  <td className="px-4 py-6">
                    <span className={cn("inline-block px-4 py-1.5 rounded-full text-[10px] font-black tracking-tight shadow-sm leading-tight", row.statusColor)}>
                      {row.status}
                    </span>
                  </td>
                  <td className="px-4 py-6">
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="text-[10px] font-bold text-slate-400 tracking-tight">{row.spec}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center justify-end gap-5">
                      <button className="text-primary text-[10px] font-black hover:scale-105 transition-all tracking-widest">{t('expiry.action_transfer')}</button>
                      <button className="text-amber-500 text-[10px] font-black hover:scale-105 transition-all tracking-widest">{t('expiry.action_promo')}</button>
                      <button className="text-rose-500 text-[10px] font-black hover:scale-105 transition-all tracking-widest">{t('expiry.action_return')}</button>
                    </div>
                  </td>
                </tr>
              ))}
